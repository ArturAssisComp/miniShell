...
static bool token_array (size_t *current_index_address, struct L_token_array *array_of_tokens, struct P_command_pipeline_linked_list *command_pipeline_linked_list, char error_msg[P_ERROR_MSG_SZ])
{
    ...
    switch(current_token_type)
    {
        ...
        case NEW_LINE:
            result = false;
            snprintf(error_msg, P_ERROR_MSG_SZ, "Syntax error: Unexpected token. The %lu-th token (\"%s\") is unexpected. Expected ID, COMMENT or EOF_TOKEN as next token.", *current_index_address + 1, get_token_type_str(current_token_type));
            break;
        default:
            result = false;
...
static bool content (size_t *current_index_address, struct L_token_array *array_of_tokens, struct P_command_pipeline_node *command_pipeline_node, char error_msg[P_ERROR_MSG_SZ])          
{
    ...
    switch(current_token_type)
    {
        ...
        case NEW_LINE:
            result = false;
            snprintf(error_msg, P_ERROR_MSG_SZ, "Syntax error: Unexpected token. The %lu-th token (\"%s\") is unexpected. Expected ID as next token.", *current_index_address + 1, get_token_type_str(current_token_type));
            break;
...
static bool next_command (size_t *current_index_address, struct L_token_array *array_of_tokens, struct P_command_pipeline_node *command_pipeline_node, char error_msg[P_ERROR_MSG_SZ])     
{
    ...
    switch(current_token_type)
    {
        ...
        case ID:
            result = false;
            snprintf(error_msg, P_ERROR_MSG_SZ, "Syntax error: Unexpected token. The %lu-th token (\"%s\") is unexpected. Expected PIPE, COMMENT, EOF_TOKEN, or BACKGROUND_EXEC as next token.", *current_index_address + 1, array_of_tokens->array[current_index]->token_value);
            break;
        ...
        case NEW_LINE:
            result = false;
            snprintf(error_msg, P_ERROR_MSG_SZ, "Syntax error: Unexpected token. The %lu-th token (\"%s\") is unexpected. Expected PIPE, COMMENT, EOF_TOKEN, or BACKGROUND_EXEC as next token.", *current_index_address + 1, get_token_type_str(current_token_type));
            break;
...
static bool resources (size_t *current_index_address, struct L_token_array *array_of_tokens, struct P_command *command, char error_msg[P_ERROR_MSG_SZ])        
{
    ...
    switch(current_token_type)
    {
        ...
        case NEW_LINE:
            result = false;
            snprintf(error_msg, P_ERROR_MSG_SZ, "Syntax error: Unexpected token. The %lu-th token (\"%s\") is unexpected. Expected PIPE, ID, IN_REDIRECT, OUT_REDIRECT, COMMENT, EOF_TOKEN, or BACKGROUND_EXEC as next token.", *current_index_address + 1, get_token_type_str(current_token_type));
            break;
...
static bool resources1 (size_t *current_index_address, struct L_token_array *array_of_tokens, struct P_command *command, char error_msg[P_ERROR_MSG_SZ])       
{
    ...
    switch(current_token_type)
    {
        ...
        case IN_REDIRECT:
            result = false;
            snprintf(error_msg, P_ERROR_MSG_SZ, "Syntax error: Input stream already defined for the command \"%s\". The %lu-th token (\"%s\") is unexpected. Expected PIPE, ID, OUT_REDIRECT, COMMENT, EOF_TOKEN, or BACKGROUND_EXEC as next token.", command->id, *current_index_address + 1, get_token_type_str(current_token_type));
            break;

        case RES_WORD:
        case NEW_LINE:
            result = false;
            snprintf(error_msg, P_ERROR_MSG_SZ, "Syntax error: Unexpected token. The %lu-th token (\"%s\") is unexpected. Expected PIPE, ID, OUT_REDIRECT, COMMENT, EOF_TOKEN, or BACKGROUND_EXEC as next token.", *current_index_address + 1, get_token_type_str(current_token_type));
            break;
...
static bool argument (size_t *current_index_address, struct L_token_array *array_of_tokens, struct P_command *command, char error_msg[P_ERROR_MSG_SZ])         
{
    ...
    switch(current_token_type)
    {
        case ID:
            ...
            {
                result = false;
                snprintf(error_msg, P_ERROR_MSG_SZ, "Syntax error: the command \"%s\" (%lu-th token) exceded the maximum number of arguments allowed (P_MAX_ARGS = %d).", command->id, current_index, P_MAX_ARGS);
                goto return_result;
            }
...
static bool input_redirect (size_t *current_index_address, struct L_token_array *array_of_tokens, struct P_command *command, char error_msg[P_ERROR_MSG_SZ])   
{
    ...
    switch(current_token_type)
    {
        ...
        case NEW_LINE:
            result = false;
            snprintf(error_msg, P_ERROR_MSG_SZ, "Syntax error: Unexpected token. The %lu-th token (\"%s\") is unexpected. Expected IN_REDIRECT as next token.", *current_index_address + 1, get_token_type_str(current_token_type));
            break;
...
static bool output_redirect (size_t *current_index_address, struct L_token_array *array_of_tokens, struct P_command *command, char error_msg[P_ERROR_MSG_SZ])  
{
    ...
    switch(current_token_type)
    {
        case OUT_REDIRECT:
            ...
            {
                result = false;
                snprintf(error_msg, P_ERROR_MSG_SZ, "Syntax error: the command \"%s\" (%lu-th token) exceded the maximum number of output redirection arguments allowed (P_MAX_OUTPUT_REDIRECTION_IDS = %d).", command->id, current_index, P_MAX_OUTPUT_REDIRECTION_IDS);
                goto return_result;
            }
        ...
        case NEW_LINE:
            result = false;
            snprintf(error_msg, P_ERROR_MSG_SZ, "Syntax error: Unexpected token. The %lu-th token (\"%s\") is unexpected. Expected OUT_REDIRECT as next token.", *current_index_address + 1, get_token_type_str(current_token_type));
            break;
...
static bool modifiers (size_t *current_index_address, struct L_token_array *array_of_tokens, struct P_command_pipeline_node *command_pipeline_node, char error_msg[P_ERROR_MSG_SZ])        
{
    ...
    switch(current_token_type)
    {
        ...
        case ID:
            result = false;
            snprintf(error_msg, P_ERROR_MSG_SZ, "Syntax error: Unexpected token. The %lu-th token (\"%s\") is unexpected. Expected COMMENT, EOF_TOKEN, or BACKGROUND_EXEC as next token.", *current_index_address + 1, array_of_tokens->array[current_index]->token_value);
            break;
        ...
        case NEW_LINE:
            result = false;
            snprintf(error_msg, P_ERROR_MSG_SZ, "Syntax error: Unexpected token. The %lu-th token (\"%s\") is unexpected. Expected COMMENT, EOF_TOKEN, or BACKGROUND_EXEC as next token.", *current_index_address + 1, get_token_type_str(current_token_type));
            break;
...
static bool comment_line (size_t *current_index_address, struct L_token_array *array_of_tokens, char error_msg[P_ERROR_MSG_SZ])     
{
    ...
    switch(current_token_type)
    {
        ...
        case ID:
            result = false;
            snprintf(error_msg, P_ERROR_MSG_SZ, "Syntax error: Unexpected token. The %lu-th token (\"%s\") is unexpected. Expected COMMENT or EOF_TOKEN as next token.", *current_index_address + 1, array_of_tokens->array[current_index]->token_value);
            break;
        ...
        case NEW_LINE:
            result = false;
            snprintf(error_msg, P_ERROR_MSG_SZ, "Syntax error: Unexpected token. The %lu-th token (\"%s\") is unexpected. Expected COMMENT or EOF_TOKEN as next token.", *current_index_address + 1, get_token_type_str(current_token_type));
            break;
...
static bool end (size_t *current_index_address, struct L_token_array *array_of_tokens, char error_msg[P_ERROR_MSG_SZ])              
{
    ...
    switch(current_token_type)
    {
        ...
        case ID:
            result = false;
            snprintf(error_msg, P_ERROR_MSG_SZ, "Syntax error: Unexpected token. The %lu-th token (\"%s\") is unexpected. Expected EOF_TOKEN as next token.", *current_index_address + 1, array_of_tokens->array[current_index]->token_value);
            break;
        ...
        case NEW_LINE:
            result = false;
            snprintf(error_msg, P_ERROR_MSG_SZ, "Syntax error: Unexpected token. The %lu-th token (\"%s\") is unexpected. Expected EOF_TOKEN as next token.", *current_index_address + 1, get_token_type_str(current_token_type));
            break;
